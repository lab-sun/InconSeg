...
parser.add_argument('--weight_name', '-w', type=str, default='InconSeg_RGB_disparity') # RTFNet_152, RTFNet_50, please change the number of layers in the network file
...
if __name__ == '__main__':
    ...
    batch_size = 1
    test_dataset  = RGB_Disparity_dataset(data_dir=args.data_dir, split=args.dataset_split, input_h=args.img_height, input_w=args.img_width)
    ...
    print("* recall per class: \n    unlabeled: %.6f, positive: %.6f, negative: %.6f" \
    ...
    print("\n* average values (np.mean(x)): \n recall: %.6f, F1: %.6f, iou: %.6f" \
          %(recall_per_class[1:].mean(), F1_per_class[1:].mean(), iou_per_class[1:].mean()))
    print("* average values (np.mean(np.nan_to_num(x))): \n recall: %.6f, F1: %.6f, iou: %.6f" \
          %(np.mean(np.nan_to_num(recall_per_class[1:])), np.mean(np.nan_to_num(F1_per_class[1:])), np.mean(np.nan_to_num(iou_per_class[1:]))))
